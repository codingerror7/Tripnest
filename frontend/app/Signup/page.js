...
function App() {
  ...
  const onSubmit = async (data)=>{
    ...
    try {
      let res = await axios.post("https://tripnest-travel-app.onrender.com/api/auth/signup",data);
      localStorage.setItem("token",res.data.token);
    alert("Signup successfull");
    reset();
    router.push("/");
    console.log(res.data,{message:"signup successfull"});
    } catch (error) {
  ...
  return (
   ...
   <div className='min-h-[100vh] border-[#f8eded] border-[10px] bg-stone-900 overflow-hidden'>
     ...
     <div className='absolute w-[10vw] h-[20vh] rounded-[50%] ml-[26%] mt-[25%] bg-stone-900'></div>
          <form onSubmit={handleSubmit(onSubmit)} className='border-[1px] border-black rounded-[20px] p-[15px] w-[35%] min-h-[50vh] ml-[32.5%] mt-[7%] bg-[#f8eded]'>
            ...
            <div className='mt-[7%] ml-[15%]'>
              ...
              <input type='email' name='email' {...register('email',{required:true})} className='border-[1px] border-black p-[5px] ml-[2%] rounded-[10px] w-[70%] font-[gilroy]' placeholder='Enter your email:'></input>
            ...
            <div className='mt-[7%] ml-[8%]'>
              ...
              <input type={show?"text":"password"} {...register('password',{required:true})} name='password' className='border-[1px] border-black p-[5px] ml-[2%] rounded-[10px] w-[64%] font-[gilroy]' placeholder='Enter password:'
              ></input>
            ...
            <div className='mt-[7%]'>
              <button className='rounded-[20px] p-[5px] text-[12px] text-white bg-stone-900 font-semibold cursor-pointer ml-[35%] w-[30%]'>Signup</button>
            </div>
            <p className='text-[14px] text-black font-medium font-[gilroy] ml-[12%] mt-[4%]'>Already a user? <Link href='./Login' className='p-[5px] text-[15px] text-black font-semibold cursor-pointer w-[30%] underline'>Login</Link></p>
          </form>
